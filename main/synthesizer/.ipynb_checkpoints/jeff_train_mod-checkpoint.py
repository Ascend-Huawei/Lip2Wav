...
def train(log_dir, args, hparams):
    ...
    #with tf.Session(config=npu_config_proto(config_proto=config)) as sess:
        try:
            ...
            # ########################################################################
            # initializing feeder - enqueue in background thread whenever q.size < 8
            # ########################################################################
            ...
            # ########################################################################
            # Verify if Tacotron Graph is loaded in correctly
            # ########################################################################
#             g = sess.graph
...
#                 print(n.name)
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                # ###############################################################
                # Verify whether feeder can enqueue and dequeue
                # Dequeue feed_dict from Feeder class (do more analysis on shape and size...)
                # Problem - thread is not running the enqueue operation fast enough
                # ###############################################################
                # feed_dict = feeder.get_feed_dict()  # change this so it grabs from queue?
