...
from synthesizer.rahul_feeder import Feeder
...
def train(log_dir, args, hparams):
    ...
    #with tf.Session(config=npu_config_proto(config_proto=config)) as sess:
        try:
            ...
            ############################################
            # Verify if Tacotron is loaded in correctly
            ############################################
#             g = sess.graph
#             g_def = g.as_graph_def()
#             print("View nodes in GraphDef...")
#             for n in g_def.node:
#                 print(n.name)
            # ###########################################
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                print ("optimize : ", model.optimize) #remove opt and try

                # test run forward
                # var_recorder = VarRecorder.instance()
                # vars = var_recorder.get_vars()
                # names, tensors  = zip(*vars)
                # print("--------------------")
                # print(names) 
                # print(tensors)
                # print("--------------------")
                ...
                # ###############################################################
                # Verify whether feed_dict is correctly passed in as inputs
                # feed_dict - correctly getting the batch from Feeder class (do more analysis on shape and size...)
                # Dequeue from feeder first
                # ###############################################################
                feed_dict = feeder.get_feed_dict()  # change this so it grabs from queue - or not?
                # feed_dict.keys() = 6 tf.placeholder defined in Feeder._placeholder
                # feed_dict.values() = batch 
                print("Feed_dict")
#                 print("FD Keys: ", feed_dict.keys()) 
#                 print("FD Values: ", feed_dict.values())
                input_key_list = list(feed_dict.keys())
                input_val_list = list(feed_dict.values())
                for ind, input_val in enumerate(input_val_list):
                    print(input_key_list[ind])
                    print(f"Value Type: {type(input_val)}")
                    print(f"Shape: {input_val.shape}")

                sess.run(feeder._enqueue_op, feed_dict=feed_dict) # this should queue numpy values to placeholder in queue
                step, loss = sess.run([global_step, model.loss, model.optimize])
#                 print(f"Step/Loss: {step}, {loss}")
                # get feed_dict from disk
#                 feed_dict = feeder.get_feed_dict()
#                 ops = tf.get_collection("leo_debug")
#                 print ("ops : ", ops)
#                 tensors_ = sess.run(ops, feed_dict=feed_dict)
                ## Need to call dequeue for default Python class ##
                ## feeder. start_thread ##
                ##################################################################
                
#                 step, loss = sess.run([global_step, model.loss, model.optimize],)
#                 step, loss = sess.run([global_step, model.loss, model.optimize], feed_dict=feed_dict)
                print("Step/Loss: {}, {}".format(step, loss))
                print ("sess run out")
#                 print (tensors_)
#                 for tensor in zip(tensors_):
#                     print(tensor)
                #step,_ = sess.run([global_step, model.loss])
                print("one step run over!")
                break
                #step, loss = sess.run([global_step, model.loss, model.optimize]) #, #opt model.optimize])
