...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, embed_targets, mel_targets=None, 
        ...
        print ("split device : ", split_device)
        ...
        with tf.device(split_device):
            ...
            print ("inputs shape inference : ", inputs.shape)
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                ...
                with tf.variable_scope("inference") as scope:
                    ...
                    decoder_state = decoder_cell.zero_state(batch_size=batch_size,
                                                                 dtype=tf.float32)  #decoder_init_state -> decoder_state
                    
                    # Only use max iterations at synthesis time
                    max_iters = hp.max_iters if not (is_training or is_evaluating) else None

                    ## TODO: Testing while loop changes
                    # Instance for CustomDecoder
                    custom_decoder = CustomDecoder(decoder_cell, self.helper, decoder_state)
                
                    # Varibale Declaration 
                    # step = 0
                    # #print("helper shape : ", self.helper._targets)
                    target_len = int(hp.mel_step_size/hp.outputs_per_step) #->120 #int(self.helper._targets.shape[1]) #int(mel_targets.shape[1])
                    ...
                    # final_output = tf.zeros([batch_size, target_len, hp.num_mels * hp.outputs_per_step], tf.float32) #[batch_size, target_len//hp.outputs_per_step, hp.num_mels * hp.outputs_per_step], tf.float32)
                    # #flag = True
                    ...
                    #res_loop = tf.while_loop(att_condition, att_body, loop_vars=[step, f_inputs, outputs, final_output, f_decoder_state],
                    #swap_memory=False)
                    ...
                    print ("print dynamic done" )
                    
                    frames_prediction,final_decoder_state, _ = dynamic_decode(
                        hp,
                        target_len,
                        CustomDecoder(decoder_cell, self.helper, decoder_state), #decoder_state ->  decoder_init_state
                        impute_finished=False,
                        maximum_iterations=max_iters,
                        swap_memory=hp.tacotron_swap_with_cpu)
                    ...
                    #tf.add_to_collection("leo_debug", residual)
                    #self.tower_mel_targets[i]
        ...
        for i in range(hp.tacotron_num_gpus + 0): #hp.tacotron_gpu_start_idx
            log("  device:                   {}".format(i))
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0",
                ...
                with tf.variable_scope("loss") as scope:
                    if hp.mask_decoder:
                        ...
                        tf.add_to_collection('leo_debug', self.tower_mel_targets[i])
                        tf.add_to_collection('leo_debug', self.tower_decoder_output[i])
                        # Compute loss after postnet
